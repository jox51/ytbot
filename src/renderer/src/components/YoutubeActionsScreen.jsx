...
const YoutubeActionsScreen = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const filePath = await window.api.getResourcePath('youtube_actions3.csv')
      setCsvPath(filePath)
  ...
  const handleSaveAll = async () => {
    try {
      ...
      await window.api.writeFile('youtube_actions3.csv', csv)
  ...
  const handleDeleteRow = (row) => {
    if (window.confirm(`Are you sure you want to delete this YouTube action?`)) {
  ...
  return (
    <div className="h-full flex flex-col">
      <h1 className="text-2xl font-bold mb-4">YouTube Actions</h1>
...
export default YoutubeActionsScreen
